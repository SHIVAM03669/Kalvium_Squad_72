  ...
  const teamMembers = [
    {
      ...
      description: 'Full-stack developer with expertise in React and Node.js. Led the development of Squad 72 website.'
    },
    {
      name: 'Gaurav Khandelwal',
      role: 'UI/UX Designer',
      photo: '/gayravv.png',
      description: 'Creative designer focused on user experience and interface design. Created the visual identity of Squad 72.'
    },
    {
      name: 'Heramb Inamke',
      role: 'Frontend Developer',
      photo: '/vanch.jpg',
      description: 'Frontend specialist with a passion for creating responsive and interactive web experiences.'
    }
  ];
    ...
    <section class="pt-32 pb-20 px-4 relative overflow-hidden">
      <div class="container mx-auto relative z-10 max-w-6xl">
        ...
        <div class="max-w-4xl mx-auto space-y-12">
          ...
          <!-- Team Section -->
          <div class="bg-gray-800/50 backdrop-blur-lg rounded-xl p-8 shadow-xl border border-gray-700">
            <h3 class="text-2xl font-bold mb-8 text-white text-center">Meet the Creators</h3>
            <div class="grid md:grid-cols-3 gap-8">
              ${teamMembers.map(member => `
                <div class="bg-gray-800/50 backdrop-blur-lg rounded-lg p-6 border border-gray-700 hover:shadow-lg transition-all duration-300">
                  <div class="w-32 h-32 mx-auto mb-4 rounded-full overflow-hidden border-4 border-[#FF3939]">
                    <img src="${member.photo}" alt="${member.name}" class="w-full h-full object-cover">
                  </div>
                  <h4 class="text-xl font-semibold text-white text-center mb-2">${member.name}</h4>
                  <p class="text-[#FF3939] text-center mb-3">${member.role}</p>
                  <p class="text-gray-400 text-center text-sm">${member.description}</p>
                </div>
              `).join('')}
            </div>
          </div>
